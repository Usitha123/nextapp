...
const OwnerTable = () => {
  ...
  const handleDelete = useCallback(async () => {
    ...
    try {
      ...
      console.error(error);
      alert(error.message || "Error deleting owner");
    } finally {
      setLoading(false);
    }
  ...
  if (loading && owners.length === 0) return <div className="flex items-center justify-center p-8 text-gray-400">Loading owners...</div>;
  if (error) return (
    <div className="p-4 text-red-400 rounded-lg bg-red-900/20">
      <p>Error: {error}</p>
      <button onClick={fetchOwners} className="px-4 py-2 mt-2 text-white bg-red-600 rounded hover:bg-red-700 transition">Retry</button>
    </div>
  );
  ...
  return (
    <div className="p-2 rounded-lg">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-xl font-semibold text-gray-200">Owner Management</h2>
          <p className="mt-1 text-sm text-gray-400">Manage canteen owners and their permissions</p>
        </div>
        <Link href="/admindashboard/User/Owner/Addowners" className="flex items-center gap-2 px-4 py-2 text-sm font-medium bg-[#3B3737] text-orange-500 border border-orange-500/50 rounded-xl hover:bg-black hover:border-orange-500 transition">
          <PlusCircle size={18} />
          Add Owner
        </Link>
      </div>
